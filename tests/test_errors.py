...
def test_invalid_bytes():
    with pytest.raises(TypioValidationError, match=r"bytes text must be UTF-8 decodable."):
...
def test_invalid_end():
    with pytest.raises(TypioValidationError, match=r"`end` must be a str."):
        type_print("test", end=1)


def test_invalid_file():
    with pytest.raises(TypioValidationError, match=r"`file` must be a file-like object."):
